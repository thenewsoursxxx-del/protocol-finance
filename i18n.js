...
(function (global) {
  ...
  var I18N = {
    ru: {
      ...
      "faq.intro": "Короткие материалы и ответы на частые вопросы о личных финансах и инвестициях. Нажмите на раздел, чтобы раскрыть его.",
      "faq.section.calc": "Расчёт",
      "faq.calc.pace.q": "Изменить темп накоплений",
      "faq.calc.pace.a": "<h4>Что это</h4><p>Темп накоплений — это доля ваших <b>свободных средств</b>, которую приложение каждый месяц направляет на цель. Свободные средства считаются просто: <b>доход − расходы</b>.</p><h4>Три режима</h4><ul><li><b>Спокойно</b> — на цель идёт около <b>40%</b> свободных средств. Комфортно, без давления на бюджет.</li><li><b>Умеренно</b> — около <b>60%</b>. Баланс между скоростью и комфортом.</li><li><b>Агрессивно</b> — около <b>80%</b>. Максимальная скорость, но выше нагрузка на бюджет.</li></ul><h4>Пример</h4><p class='faq-example'>Ваш доход — 100 000 ₽, расходы — 60 000 ₽. Значит, свободно остаётся 40 000 ₽.<br>• Спокойно → на цель 16 000 ₽/мес.<br>• Умеренно → 24 000 ₽/мес.<br>• Агрессивно → 32 000 ₽/мес.<br>Чем выше темп, тем быстрее вы достигнете цели, но тем меньше денег остаётся «на жизнь».</p><h4>Как изменить</h4><p>Нажмите кнопку <b>«Изменить темп накоплений»</b> на карточке плана. Выберите новый режим — приложение сразу покажет предпросмотр: новый объём накоплений в месяц и новый срок достижения цели. Если всё устраивает — нажмите «Сохранить темп накоплений».</p><p>Например: вы были на «Спокойно», и цель достигалась за 30 месяцев. Переключаете на «Агрессивно» — приложение пересчитает и покажет, что теперь цель будет достигнута примерно за 15 месяцев.</p>",
      "faq.calc.debts.q": "Добавить кредиты и долги",
      "faq.calc.debts.a": "<h4>Что это</h4><p>Раздел, где вы вносите свои кредиты и долги, чтобы план учитывал их наравне с накоплениями. Для каждого долга указываются сумма, ежемесячный платёж и другие параметры.</p><h4>Автопогашение из накоплений</h4><p>Главный переключатель раздела определяет, как долги влияют на план:</p><ul><li><b>Включено</b> — часть суммы, которую вы откладываете, автоматически идёт на погашение долгов <b>в первую очередь</b>, а остаток — в резерв и на цель.</li><li><b>Выключено</b> — погашение долгов фиксируется отдельно и <b>не уменьшает</b> ваши накопления автоматически.</li></ul><h4>Пример</h4><p class='faq-example'>Автопогашение включено. Вы откладываете 20 000 ₽, а по долгам в этом месяце нужно внести 8 000 ₽.<br>→ 8 000 ₽ уйдёт на погашение долга, оставшиеся 12 000 ₽ — в резерв и на цель.<br>Когда долг полностью закрыт, он перестаёт участвовать в распределении, и вся сумма снова идёт на цель.</p><p>Если долгов несколько, платежи распределяются по текущим обязательствам по очереди, пока хватает отложенной суммы.</p>",
      "faq.calc.flex.q": "Гибкая модель",
      "faq.calc.flex.a": "<h4>Зачем нужна</h4><p>Обычный расчёт предполагает, что доход и расходы фиксированы и одинаковы каждый месяц. Но в реальности так не у всех: зарплата приходит частями, доход зависит от заказов, расходы скачут. <b>Гибкая модель</b> строит прогноз по вашему реальному графику поступлений и трат — поэтому срок достижения цели становится точнее.</p><p>Открывается кнопкой <b>«Гибкая финансовая модель»</b> на экране расчёта. Внутри — три шага: доход, расходы и разовые доходы.</p><h4>Шаг 1. Доход: фиксированный или нефиксированный</h4><p><b>Фиксированный</b> — вы получаете одну и ту же сумму (берётся из поля «Доход»). Дополнительно вводить ничего не нужно.</p><p><b>Нефиксированный</b> — сумма и/или частота дохода меняются. Здесь вы выбираете, как часто приходят деньги:</p><ul><li><b>Ежемесячно</b> — раз в месяц.</li><li><b>Раз в неделю</b> и <b>Раз в 2 недели</b> — регулярные выплаты чаще, чем раз в месяц.</li><li><b>Свой график</b> — нерегулярный доход: вы записываете каждое поступление вручную.</li></ul><p class='faq-example'>Пример: вы фрилансер, оплата приходит нерегулярно. Выбираете «Нефиксированный» → «Свой график». Теперь каждое поступление вы записываете тогда, когда оно реально пришло.</p><h4>Шаг 2. Расходы: фиксированные или нефиксированные</h4><p>Всё так же, как с доходом. <b>Фиксированные</b> — постоянная сумма (из поля «Расходы»). <b>Нефиксированные</b> — выбираете частоту (ежемесячно / неделя / 2 недели / свой график) и записываете траты по графику.</p><h4>Как считаются свободные средства</h4><p>За период приложение считает: <b>свободно = доход − расходы</b>. На цель идёт доля от свободного согласно выбранному темпу (см. раздел «Изменить темп накоплений»).</p><p class='faq-example'>Пример: за месяц доход 80 000 ₽, расходы 50 000 ₽ → свободно 30 000 ₽. При темпе «Умеренно» (60%) приложение рекомендует отложить на цель 18 000 ₽.</p><h4>Частоты «неделя / 2 недели / месяц»</h4><p>Это <b>регулярные</b> графики. Приложение строит прогноз, считая, что выплаты повторяются с выбранной частотой. Когда вы записываете поступление, доля от него по темпу сразу откладывается на цель.</p><h4>Режим «Свой график» — подробно</h4><p>Это режим для <b>нерегулярных</b> денег. Здесь действует простое и надёжное правило: <b>«записать» и «отложить на цель» — это два разных действия</b>.</p><p>Как это работает по шагам:</p><ul><li>Кнопками на графике <b>«Записать доход»</b> и <b>«Записать расход»</b> вы просто ведёте журнал за календарный месяц — в любом порядке и сколько угодно раз.</li><li>Приложение показывает: доход за месяц, расходы за месяц, «свободно» и «нужно отложить».</li><li>Деньги уходят на цель <b>только</b> когда вы нажимаете кнопку <b>«Отложить»</b>. Автоматически ничего не откладывается.</li><li>В начале нового месяца счётчики месяца обнуляются, а уже отложенное на цель остаётся на счёте.</li></ul><p class='faq-example'>Пример: пришла зарплата — записываете доход 50 000 ₽. Приложение показывает «нужно отложить 30 000 ₽» (при вашем темпе). В течение месяца записываете расходы, например 10 000 ₽ — сумма «нужно отложить» сразу пересчитывается вниз. Когда будете готовы — нажимаете «Отложить», и ровно эта сумма попадает на счёт цели. Так вы никогда не отложите лишнего.</p><p>Порядок ввода не важен: можно сначала записать расходы, а потом доход — итог будет тот же.</p><h4>Разовые доходы</h4><p>Отдельный блок для непредсказуемых поступлений: премия, подарок, возврат долга, продажа вещей, подработка. Они добавляются разово и увеличивают прогноз, не ломая ваш регулярный график.</p><h4>Связь с вкладкой «Расходы»</h4><p>В форме добавления расхода есть тумблер <b>«Учитывать расходы в плане»</b>. Если он включён, этот расход тоже уменьшит сумму, доступную для цели в этом месяце. В режиме «Свой график» расходы можно вводить прямо на графике — поэтому, чтобы один и тот же расход не посчитался дважды, ведите его в чём-то одном: либо на графике, либо во вкладке «Расходы».</p><h4>Резерв</h4><p>Если у вас выбран план с резервом («буфер»), примерно <b>10%</b> каждого отложения идёт в резервный счёт — это ваша подушка безопасности, а остальное — на цель.</p>",
      "faq.section.dividends": "Дивиденды",
      "faq.dividends.what.q": "Что такое дивиденды?",
      "faq.dividends.what.a": "Дивиденды - это часть прибыли компании, которую она выплачивает своим акционерам. Покупая акцию, вы становитесь совладельцем бизнеса, и если компания решает распределить прибыль, вы получаете выплату пропорционально числу ваших акций.",
      "faq.dividends.what.caption": "Пример: у вас 100 акций, дивиденд - 5 ₽ на акцию → вы получите 500 ₽.",
      "faq.dividends.freq.q": "Как часто выплачивают дивиденды?",
      "faq.dividends.freq.a": "Периодичность зависит от компании и страны. Чаще всего дивиденды платят раз в год, раз в полгода или раз в квартал. Некоторые компании не платят дивиденды вовсе, а направляют прибыль на развитие бизнеса.",
      "faq.dividends.reinvest.q": "Что такое реинвестирование дивидендов?",
      "faq.dividends.reinvest.a": "Реинвестирование - это когда полученные дивиденды вы снова вкладываете в покупку акций, а не тратите. Со временем это работает как сложный процент: доход приносит новый доход, и капитал растёт быстрее.",
      ...
      "stats.asset.ru_novatek":    "Новатэк (NVTK)",
      "stats.asset.ru_tatneft":    "Татнефть (TATN)",
      "stats.asset.ru_surgut":     "Сургутнефтегаз (SNGS)",
      "stats.asset.ru_severstal":  "Северсталь (CHMF)",
      "stats.asset.ru_nlmk":       "НЛМК (NLMK)",
      "stats.asset.ru_mmk":        "ММК (MAGN)",
      "stats.asset.ru_mts":        "МТС (MTSS)",
      "stats.asset.ru_moex":       "Московская биржа (MOEX)",
      "stats.asset.ru_alrosa":     "АЛРОСА (ALRS)",
      "stats.asset.ru_phosagro":   "ФосАгро (PHOR)",
      "stats.asset.ru_aeroflot":   "Аэрофлот (AFLT)",
      "stats.asset.ru_interrao":   "Интер РАО (IRAO)",
      "stats.asset.ru_rostelecom": "Ростелеком (RTKM)",
      "stats.asset.ru_pik":        "ПИК (PIKK)",
      ...
      // ── Intro stories (вступительные сторис при входе) ──
      "stories.skip": "Пропустить",
      "stories.paused": "пауза",
      "stories.cta": "Начать",
      "stories.s1.tag": "Protocol",
      "stories.s1.title": "Деньги по плану,<br>а не на ощупь",
      "stories.s1.body": "Я посчитаю, сколько откладывать каждый месяц, и покажу, когда вы придёте к цели. Без таблиц и ручных расчётов.",
      "stories.s2.tag": "Текущий план",
      "stories.s2.title": "Срок и сумма<br>считаются сами",
      "stories.s2.body": "Доход, расходы и цель - Protocol строит график плана и факта.",
      "stories.s2.plan": "План",
      "stories.s2.fact": "Факт",
      "stories.s2.save": "Откладываете",
      "stories.s2.eta": "Цель через",
      "stories.s2.monthsShort": " мес",
      "stories.s3.tag": "С резервом",
      "stories.s3.title": "Форс-мажор<br>не сорвёт план",
      "stories.s3.body": "Каждое отложение делится: большая часть идёт в цель, десятая - в резерв. Сломалась техника - берёте из подушки, а не из цели.",
      "stories.s3.direct": "Всё в цель",
      "stories.s3.directNote": "Максимально быстро",
      "stories.s3.buffer": "С резервом",
      "stories.s3.bufferNote": "Со страховкой",
      "stories.s3.reserve": "Подушка безопасности",
      "stories.s4.tag": "Расходы",
      "stories.s4.title": "Видно, куда<br>уходят деньги",
      "stories.s4.body": "Девять категорий и разбивка по месяцам. Одним переключателем траты начинают учитываться в плане.",
      "stories.s4.spent": "Потрачено",
      "stories.s5.tag": "Premium · Свой график",
      "stories.s5.title": "Доход рывками -<br>план не ломается",
      "stories.s5.body": "Записываете приход или трату в тот день, когда они случились. Protocol пересчитывает «Свободно», а в цель деньги уходят только по кнопке «Отложить».",
    ...
    en: {
      ...
      "faq.intro": "Short guides and answers to common questions about personal finance and investing. Tap a section to expand it.",
      "faq.section.calc": "Planning",
      "faq.calc.pace.q": "Change your saving pace",
      "faq.calc.pace.a": "<h4>What it is</h4><p>Your saving pace is the share of your <b>free funds</b> that the app puts toward your goal every month. Free funds are calculated simply: <b>income − expenses</b>.</p><h4>Three modes</h4><ul><li><b>Relaxed</b> — about <b>40%</b> of free funds go to the goal. Comfortable, no pressure on your budget.</li><li><b>Moderate</b> — about <b>60%</b>. A balance between speed and comfort.</li><li><b>Aggressive</b> — about <b>80%</b>. Maximum speed, but a heavier load on your budget.</li></ul><h4>Example</h4><p class='faq-example'>Your income is 100,000 ₽ and expenses are 60,000 ₽, so 40,000 ₽ stays free.<br>• Relaxed → 16,000 ₽/mo to the goal.<br>• Moderate → 24,000 ₽/mo.<br>• Aggressive → 32,000 ₽/mo.<br>The higher the pace, the faster you reach the goal, but the less money is left for living.</p><h4>How to change it</h4><p>Tap <b>“Change your saving pace”</b> on the plan card. Pick a new mode and the app instantly shows a preview: the new monthly amount and the new time to reach your goal. If it looks good, tap “Save saving pace”.</p><p>For example: you were on “Relaxed” and the goal took 30 months. Switch to “Aggressive” and the app recalculates, showing the goal is now reached in about 15 months.</p>",
      "faq.calc.debts.q": "Add loans and debts",
      "faq.calc.debts.a": "<h4>What it is</h4><p>A section where you enter your loans and debts so the plan accounts for them alongside your savings. For each debt you set the amount, the monthly payment and other details.</p><h4>Auto-repayment from savings</h4><p>The main toggle in this section decides how debts affect the plan:</p><ul><li><b>On</b> — part of the amount you set aside automatically goes to debt repayment <b>first</b>, and the rest goes to the reserve and the goal.</li><li><b>Off</b> — debt repayment is tracked separately and does <b>not</b> reduce your savings automatically.</li></ul><h4>Example</h4><p class='faq-example'>Auto-repayment is on. You set aside 20,000 ₽, and 8,000 ₽ is due on your debts this month.<br>→ 8,000 ₽ goes to debt repayment, the remaining 12,000 ₽ goes to the reserve and the goal.<br>Once a debt is fully paid off, it stops taking part in the split and the full amount goes to the goal again.</p><p>If you have several debts, payments are spread across the current obligations one by one, as long as the set-aside amount lasts.</p>",
      "faq.calc.flex.q": "Flexible model",
      "faq.calc.flex.a": "<h4>Why you need it</h4><p>The standard calculation assumes your income and expenses are fixed and the same every month. But that is not the case for everyone: salary arrives in parts, income depends on orders, expenses jump around. The <b>flexible model</b> builds a forecast from your real schedule of income and spending — so the time to reach your goal becomes more accurate.</p><p>It opens with the <b>“Flexible financial model”</b> button on the planning screen. Inside there are three steps: income, expenses and one-off income.</p><h4>Step 1. Income: fixed or variable</h4><p><b>Fixed</b> — you receive the same amount (taken from the “Income” field). Nothing extra to enter.</p><p><b>Variable</b> — the amount and/or frequency of income changes. Here you choose how often money arrives:</p><ul><li><b>Monthly</b> — once a month.</li><li><b>Weekly</b> and <b>Every 2 weeks</b> — regular payments more often than once a month.</li><li><b>Custom schedule</b> — irregular income: you log each payment manually.</li></ul><p class='faq-example'>Example: you are a freelancer and get paid irregularly. Choose “Variable” → “Custom schedule”. Now you log each payment when it actually arrives.</p><h4>Step 2. Expenses: fixed or variable</h4><p>Same as with income. <b>Fixed</b> — a constant amount (from the “Expenses” field). <b>Variable</b> — you pick a frequency (monthly / weekly / every 2 weeks / custom schedule) and log spending on that schedule.</p><h4>How free funds are calculated</h4><p>For a period the app calculates: <b>free = income − expenses</b>. A share of the free amount goes to the goal based on your chosen pace (see “Change your saving pace”).</p><p class='faq-example'>Example: over a month income is 80,000 ₽ and expenses are 50,000 ₽ → 30,000 ₽ is free. At the “Moderate” pace (60%) the app recommends setting aside 18,000 ₽ toward the goal.</p><h4>Frequencies “weekly / every 2 weeks / monthly”</h4><p>These are <b>regular</b> schedules. The app builds the forecast assuming payments repeat at the chosen frequency. When you log a payment, a share of it is set aside for the goal right away, based on your pace.</p><h4>“Custom schedule” mode — in detail</h4><p>This is the mode for <b>irregular</b> money. A simple, reliable rule applies here: <b>“logging” and “setting aside for the goal” are two different actions</b>.</p><p>How it works, step by step:</p><ul><li>With the <b>“Log income”</b> and <b>“Log expense”</b> buttons on the chart you simply keep a journal for the calendar month — in any order and as many times as you like.</li><li>The app shows: income for the month, expenses for the month, “free” and “to set aside”.</li><li>Money goes to the goal <b>only</b> when you tap the <b>“Set aside”</b> button. Nothing is set aside automatically.</li><li>At the start of a new month the monthly counters reset, while what you already set aside for the goal stays in the account.</li></ul><p class='faq-example'>Example: your salary arrives — you log income of 50,000 ₽. The app shows “set aside 30,000 ₽” (at your pace). During the month you log expenses, say 10,000 ₽ — the “to set aside” amount is immediately recalculated downward. When you are ready, you tap “Set aside” and exactly that amount goes to the goal account. This way you never set aside too much.</p><p>The order does not matter: you can log expenses first and income later — the result is the same.</p><h4>One-off income</h4><p>A separate block for unpredictable income: a bonus, a gift, a repaid loan, selling things, side work. These are added once and increase the forecast without breaking your regular schedule.</p><h4>Link with the “Expenses” tab</h4><p>The add-expense form has an <b>“Count expenses in the plan”</b> toggle. If it is on, that expense also reduces the amount available for the goal this month. In “Custom schedule” mode you can enter expenses right on the chart — so, to avoid counting the same expense twice, keep it in one place only: either on the chart or in the “Expenses” tab.</p><h4>Reserve</h4><p>If you chose a plan with a reserve (“buffer”), about <b>10%</b> of each set-aside goes into the reserve account — your safety cushion — and the rest goes to the goal.</p>",
      "faq.section.dividends": "Dividends",
      "faq.dividends.what.q": "What are dividends?",
      "faq.dividends.what.a": "Dividends are a share of a company's profit that it pays out to its shareholders. When you buy a share, you become a co-owner of the business, and if the company decides to distribute profit, you receive a payout proportional to the number of shares you hold.",
      "faq.dividends.what.caption": "Example: you own 100 shares, the dividend is 5 ₽ per share → you receive 500 ₽.",
      "faq.dividends.freq.q": "How often are dividends paid?",
      "faq.dividends.freq.a": "The frequency depends on the company and the country. Most often dividends are paid once a year, twice a year, or quarterly. Some companies don't pay dividends at all and reinvest profit into growing the business.",
      "faq.dividends.reinvest.q": "What is dividend reinvesting?",
      "faq.dividends.reinvest.a": "Reinvesting is when you use the dividends you receive to buy more shares instead of spending them. Over time this works like compound interest: income generates new income, and your capital grows faster.",
      ...
      "stats.asset.ru_novatek":    "Novatek (NVTK)",
      "stats.asset.ru_tatneft":    "Tatneft (TATN)",
      "stats.asset.ru_surgut":     "Surgutneftegas (SNGS)",
      "stats.asset.ru_severstal":  "Severstal (CHMF)",
      "stats.asset.ru_nlmk":       "NLMK (NLMK)",
      "stats.asset.ru_mmk":        "MMK (MAGN)",
      "stats.asset.ru_mts":        "MTS (MTSS)",
      "stats.asset.ru_moex":       "Moscow Exchange (MOEX)",
      "stats.asset.ru_alrosa":     "Alrosa (ALRS)",
      "stats.asset.ru_phosagro":   "PhosAgro (PHOR)",
      "stats.asset.ru_aeroflot":   "Aeroflot (AFLT)",
      "stats.asset.ru_interrao":   "Inter RAO (IRAO)",
      "stats.asset.ru_rostelecom": "Rostelecom (RTKM)",
      "stats.asset.ru_pik":        "PIK (PIKK)",
      ...
      "stories.cta": "Get started",
      "stories.s1.tag": "Protocol",
      "stories.s1.title": "Money on a plan,<br>not on a hunch",
      "stories.s1.body": "I work out how much to set aside every month and show you when you will reach your goal. No spreadsheets, no manual math.",
      "stories.s2.tag": "Current plan",
      "stories.s2.title": "The amount and date<br>are worked out for you",
      "stories.s2.body": "Income, expenses and a goal - Protocol charts plan against actual.",
      "stories.s2.plan": "Plan",
      "stories.s2.fact": "Actual",
      "stories.s2.save": "You set aside",
      "stories.s2.eta": "Goal in",
      "stories.s2.monthsShort": " mo",
      "stories.s3.tag": "With reserve",
      "stories.s3.title": "An emergency<br>won't derail the plan",
      "stories.s3.body": "Every contribution splits up: most of it goes to the goal, a tenth to the reserve. Something breaks - you take it from the cushion, not from your goal.",
      "stories.s3.direct": "All to goal",
      "stories.s3.directNote": "As fast as possible",
      "stories.s3.buffer": "With reserve",
      "stories.s3.bufferNote": "With a safety net",
      "stories.s3.reserve": "Emergency cushion",
      "stories.s4.tag": "Expenses",
      "stories.s4.title": "See where<br>the money goes",
      "stories.s4.body": "Nine categories and a month by month breakdown. One toggle makes your spending count towards the plan.",
      "stories.s4.spent": "Spent",
      "stories.s5.tag": "Premium · Custom schedule",
      "stories.s5.title": "Income in bursts -<br>the plan still holds",
      "stories.s5.body": "You log income or a purchase on the day it happens. Protocol recalculates what is Available, and money reaches the goal only when you tap Allocate.",
